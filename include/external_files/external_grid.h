...
struct RecipeMap {
    RecipeMap(int id, double min, double max) : m_id(id), m_min(min), m_max(max) {}
...
struct SingleExternalGridInfo {
    ...
    void setParams(double xMin, double xStep, double xMax, double yMin, double yStep, double yMax,
                   QVector<RecipeMap> rMaps) {
        m_x_min = xMin;
        m_x_step = xStep;
        m_y_min = yMin;
        m_y_step = yStep;
