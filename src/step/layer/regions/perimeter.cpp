...
void Perimeter::optimize(int layerNumber, Point& current_location, QVector<Path>& innerMostClosedContour,
    ...
    if (m_sb->setting<bool>(Constants::ProfileSettings::SpecialModes::kEnableSpiralize)) {
        if (m_computed_geometry.size() > 0) {
            poo.setGeometryToEvaluate(m_computed_geometry, RegionType::kPerimeter,
        ...
        if (static_cast<PrintDirection>(m_sb->setting<int>(
                Constants::ProfileSettings::Ordering::kPerimeterReverseDirection)) != PrintDirection::kReverse_off)
            ...
            if (newPath.size() > 0) {
                ...
                if (shouldTilt || shouldRotate) {
                    Point rotation_origin =
                        Point(m_sb->setting<Distance>(Constants::ExperimentalSettings::RotationOrigin::kXOffset),
...
void Perimeter::calculateModifiers(Path& path, bool supportsG3, QVector<Path>& innerMostClosedContour) {
    ...
    if (m_sb->setting<bool>(Constants::MaterialSettings::Slowdown::kPerimeterEnable)) {
        PathModifierGenerator::GenerateSlowdown(
            path, m_sb->setting<Distance>(Constants::MaterialSettings::Slowdown::kPerimeterDistance),
    ...
    if (m_sb->setting<bool>(Constants::MaterialSettings::TipWipe::kPerimeterEnable)) {
        if (static_cast<TipWipeDirection>(m_sb->setting<int>(
                Constants::MaterialSettings::TipWipe::kPerimeterDirection)) == TipWipeDirection::kForward ||
            static_cast<TipWipeDirection>(m_sb->setting<int>(
                Constants::MaterialSettings::TipWipe::kPerimeterDirection)) == TipWipeDirection::kOptimal)
            PathModifierGenerator::GenerateTipWipe(
                path, PathModifiers::kForwardTipWipe,
                m_sb->setting<Distance>(Constants::MaterialSettings::TipWipe::kPerimeterDistance),
                ...
                m_sb->setting<Distance>(Constants::MaterialSettings::TipWipe::kPerimeterCutoffDistance));
                     ...
                     Constants::MaterialSettings::TipWipe::kPerimeterDirection)) == TipWipeDirection::kAngled) {
            PathModifierGenerator::GenerateTipWipe(
    ...
    if (m_sb->setting<bool>(Constants::MaterialSettings::Startup::kPerimeterEnable)) {
        if (m_sb->setting<bool>(Constants::MaterialSettings::Startup::kPerimeterRampUpEnable)) {
            PathModifierGenerator::GenerateInitialStartupWithRampUp(
                path, m_sb->setting<Distance>(Constants::MaterialSettings::Startup::kPerimeterDistance),
    ...
    if (m_sb->setting<bool>(Constants::ProfileSettings::Perimeter::kEnableFlyingStart)) {
        PathModifierGenerator::GenerateFlyingStart(
            path, m_sb->setting<Distance>(Constants::ProfileSettings::Perimeter::kFlyingStartDistance),
            m_sb->setting<Velocity>(Constants::ProfileSettings::Perimeter::kFlyingStartSpeed));
    }
}

Path Perimeter::createPath(Polyline line) {
    const Point origin(m_sb->setting<double>(Constants::PrinterSettings::Dimensions::kXOffset),
    ...
    for (int j = 0; j < end_cond; ++j) {
        ...
        if (intersections.size() > 0) {
            ...
            for (Point& point : intersections) {
                ...
                segment->getSb()->setSetting(Constants::SegmentSettings::kWidth,
                                             is_settings_region ? start.getSettings()->setting<Distance>(
                                                                      Constants::ProfileSettings::Perimeter::kBeadWidth)
                                                                : default_width);
                ...
                segment->getSb()->setSetting(Constants::SegmentSettings::kSpeed,
                                             is_settings_region ? start.getSettings()->setting<Velocity>(
                                                                      Constants::ProfileSettings::Perimeter::kSpeed)
                                                                : default_speed);
                segment->getSb()->setSetting(Constants::SegmentSettings::kAccel,
                                             is_settings_region
                                                 ? start.getSettings()->setting<Acceleration>(
                                                       Constants::PrinterSettings::Acceleration::kPerimeter)
                                                 : default_acceleration);
                segment->getSb()->setSetting(Constants::SegmentSettings::kExtruderSpeed,
                                             is_settings_region
                                                 ? start.getSettings()->setting<AngularVelocity>(
                                                       Constants::ProfileSettings::Perimeter::kExtruderSpeed)
        ...
        segment->getSb()->setSetting(
            ...
            is_settings_region ? start.getSettings()->setting<Distance>(Constants::ProfileSettings::Layer::kLayerHeight)
                               : default_height);
        segment->getSb()->setSetting(
            Constants::SegmentSettings::kSpeed,
            is_settings_region ? start.getSettings()->setting<Velocity>(Constants::ProfileSettings::Perimeter::kSpeed)
                               : default_speed);
        segment->getSb()->setSetting(Constants::SegmentSettings::kAccel,
                                     is_settings_region ? start.getSettings()->setting<Acceleration>(
                                                              Constants::PrinterSettings::Acceleration::kPerimeter)
                                                        : default_acceleration);
        segment->getSb()->setSetting(Constants::SegmentSettings::kExtruderSpeed,
                                     is_settings_region ? start.getSettings()->setting<AngularVelocity>(
                                                              Constants::ProfileSettings::Perimeter::kExtruderSpeed)
                                                        : default_extruder_speed);
        segment->getSb()->setSetting(Constants::SegmentSettings::kMaterialNumber, material_number);
        segment->getSb()->setSetting(Constants::SegmentSettings::kRegionType, RegionType::kPerimeter);
...
void Perimeter::createSinglePaths() {
    ...
    AngularVelocity perim_extruder_speed =
        m_sb->setting<AngularVelocity>(Constants::ProfileSettings::Perimeter::kExtruderSpeed);
    // int material_number                   = m_sb->setting< int
    // >(Constants::MaterialSettings::MultiMaterial::kPerimterNum);
    ...
    AngularVelocity inset_extruder_speed =
        ...
        for (SinglePath::Polygon polygon : polygonList) {
            ...
            for (int i = 0; i < polygon.size() - 1; i++) {
                ...
                if (start.getRegionType() != end.getRegionType()) // This bridge jumps regions
                {
                    ...
                    // segment->setMaterialNumber(material_number);
                    segment->getSb()->setSetting(Constants::SegmentSettings::kRegionType,
                                                 (start.getRegionType() == SinglePath::RegionType::kPerimeter)
            ...
            new_path.append(segment);
            if (new_path.calculateLength() >
                m_sb->setting<Distance>(Constants::ProfileSettings::Perimeter::kMinPathLength))
