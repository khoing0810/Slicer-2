...
SettingTimeSpinBox::SettingTimeSpinBox(SettingTab* parent, QSharedPointer<SettingsBase> sb, QString key, fifojson json,
                                       QGridLayout* layout, int index)
    : SettingDoubleSpinBox(parent, sb, key, json, layout, index) {
    Time cur;
    m_warn = false;
    if (sb->contains(key))
        cur = sb->setting<Time>(key);
    else {
        cur = json[Constants::Settings::Master::kDefault].get<Time>();
        sb->setSetting(key, cur);
    }

    Time unit = PreferencesManager::getInstance()->getTimeUnit();
    ...
    // Set setting units
    m_unit_label->setText(PreferencesManager::getInstance()->getTimeUnitText());
}

SettingRowBase* SettingTimeSpinBox::createInstance(SettingTab* parent, QSharedPointer<SettingsBase> sb, QString key,
