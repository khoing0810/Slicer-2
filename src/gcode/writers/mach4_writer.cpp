...
QString Mach4Writer::writeInitialSetup(Distance minimum_x, Distance minimum_y, Distance maximum_x, Distance maximum_y,
    ...
    if (m_sb->setting<int>(PRS::GCode::kEnableStartupCode)) {
        ...
        if (m_sb->setting<int>(MS::Temperatures::kBed) > 0) {
            rv += "M190 P" % QString::number(m_sb->setting<Temperature>(MS::Temperatures::kBed).to(degC)) %
                  commentSpaceLine("SET BED TEMPERATURE AND WAIT");
        }

        rv += "M104 P" % QString::number(m_sb->setting<Temperature>(MS::Temperatures::kExtruder0).to(degC)) %
              commentSpaceLine("SET EXTRUDER TEMPERATURE");
        if (m_sb->setting<int>(MS::Temperatures::kBed) > 0) {
            rv += "M109 P" % QString::number(m_sb->setting<Temperature>(MS::Temperatures::kExtruder0).to(degC)) %
...
QString Mach4Writer::writeArc(const Point& start_point, const Point& end_point, const Point& center_point,
