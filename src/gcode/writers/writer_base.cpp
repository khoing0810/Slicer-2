...
QString WriterBase::writeSlicerHeader(const QString& syntax) {
    ...
    if (syntax == PRS::SyntaxString::kIngersoll)
...
QString WriterBase::writeSettingsHeader(GcodeSyntax syntax) {
    ...
    if (syntax != GcodeSyntax::kCincinnati && syntax != GcodeSyntax::kHaasInch && syntax != GcodeSyntax::kMeld &&
        ...
        text += commentLine(
            QString("Printer Base Offset: %0in").arg(m_sb->setting<Distance>(PRS::Dimensions::kZOffset).to(in)));
    }
    if (m_sb->setting<int>(PRS::Dimensions::kEnableW)) {
        text += commentLine(
    ...
    if (syntax != GcodeSyntax::kCincinnati && syntax != GcodeSyntax::kHaasInch && syntax != GcodeSyntax::kMeld &&
        ...
        text += commentLine(QString("Layer Height: %0in").arg(m_sb->setting<Distance>(PS::Layer::kLayerHeight).to(in)));
        text += commentLine(
            QString("Default Extrusion Width: %0in").arg(m_sb->setting<Distance>(PS::Layer::kBeadWidth).to(in)));
    }
    ...
    if (m_sb->setting<int>(PS::SpecialModes::kEnableSpiralize)) {
        ...
        if (m_sb->setting<int>(PS::Perimeter::kEnable))
            text += commentLine(QString("Perimeter Count: %0").arg(m_sb->setting<int>(PS::Perimeter::kCount)));
        if (m_sb->setting<int>(PS::Inset::kEnable))
            text += commentLine(QString("Inset Count: %0").arg(m_sb->setting<int>(PS::Inset::kCount)));
        if (m_sb->setting<int>(PS::Skin::kEnable))
            text += commentLine(QString("Upskin Count: %0").arg(m_sb->setting<int>(PS::Skin::kTopCount)));
        if (m_sb->setting<int>(PS::Skin::kEnable))
            text += commentLine(QString("Downskin Count: %0").arg(m_sb->setting<int>(PS::Skin::kBottomCount)));
        if (m_sb->setting<int>(PS::Skin::kEnable) &&
            (m_sb->setting<int>(PS::Skin::kTopCount) > 0 || m_sb->setting<int>(PS::Skin::kBottomCount) > 0)) {
            int skinPattern = m_sb->setting<int>(PS::Skin::kPattern);
            if (skinPattern)
                text += commentLine(QString("Skin Pattern: Lines"));
            else
                text += commentLine(QString("Skin Patern: Concentric"));
        }
        if (m_sb->setting<int>(PS::Infill::kEnable)) {
            if (m_sb->setting<int>(PS::Infill::kManualLineSpacing)) {
                text += commentLine(QString("Infill Percentage: %0%")
                                        .arg(m_sb->setting<Distance>(PS::Layer::kBeadWidth).to(in) /
                                             m_sb->setting<Distance>(PS::Infill::kLineSpacing).to(in) * 100));
            }
            else {
                text += commentLine(QString("Infill Percentage: %0%").arg(m_sb->setting<double>(PS::Infill::kDensity)));
            }

            int infill_pattern = m_sb->setting<int>(PS::Infill::kPattern);
