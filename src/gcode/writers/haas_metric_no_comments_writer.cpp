...
namespace ORNL {
HaasMetricNoCommentsWriter::HaasMetricNoCommentsWriter(GcodeMeta meta, const QSharedPointer<SettingsBase>& sb)
    : WriterBase(meta, sb) {}
...
QString HaasMetricNoCommentsWriter::writeInitialSetup(Distance minimum_x, Distance minimum_y, Distance maximum_x,
    ...
    if (m_sb->setting<int>(PRS::GCode::kEnableBoundingBox)) {
        rv += "G0 Z0" % m_newline % m_G0 % m_x % QString::number(minimum_x.to(m_meta.m_distance_unit), 'f', 4) % " Y" %
              QString::number(minimum_y.to(m_meta.m_distance_unit), 'f', 4) % m_newline % m_G0 % m_x %
              QString::number(maximum_x.to(m_meta.m_distance_unit), 'f', 4) % " Y" %
              QString::number(minimum_y.to(m_meta.m_distance_unit), 'f', 4) % m_newline % m_G0 % m_x %
              QString::number(maximum_x.to(m_meta.m_distance_unit), 'f', 4) % " Y" %
              QString::number(maximum_y.to(m_meta.m_distance_unit), 'f', 4) % m_newline % m_G0 % m_x %
              QString::number(minimum_x.to(m_meta.m_distance_unit), 'f', 4) % " Y" %
              QString::number(maximum_y.to(m_meta.m_distance_unit), 'f', 4) % m_newline % m_G0 % m_x %
              QString::number(minimum_x.to(m_meta.m_distance_unit), 'f', 4) % " Y" %
              QString::number(minimum_y.to(m_meta.m_distance_unit), 'f', 4) % m_newline % "M0" % m_newline;
...
QString HaasMetricNoCommentsWriter::writeArc(const Point& start_point, const Point& end_point,
...
QString HaasMetricNoCommentsWriter::writeShutdown() {
    ...
    rv += m_sb->setting<QString>(PRS::GCode::kEndCode) % m_newline % "M19" % m_newline % "M30" % m_newline % "%" %
          m_newline;
    return rv;
}
