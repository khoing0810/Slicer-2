...
QString MVPWriter::writeInitialSetup(Distance minimum_x, Distance minimum_y, Distance maximum_x, Distance maximum_y,
    ...
    if (m_sb->setting<int>(PRS::GCode::kEnableStartupCode)) {
        rv += "G54" % commentSpaceLine("ENABLE W01") % m_G1 % m_f %
              QString::number(m_sb->setting<Velocity>(PRS::MachineSpeed::kZSpeed).to(m_meta.m_velocity_unit), 'f', 4) %
              " Z0" % commentSpaceLine("SET INITIAL TABLE HEIGHT");
        m_current_z = 0;
    }

    if (m_sb->setting<int>(PRS::GCode::kEnableBoundingBox)) {
        rv += m_G0 % m_x % QString::number(minimum_x.to(in), 'f', 4) % m_y % QString::number(minimum_y.to(in), 'f', 4) %
...
QString MVPWriter::writeTravelLift(bool lift) {
    // Removed 10/6/2023 - Steven says slide functionality has been removed
    /*if (lift)
