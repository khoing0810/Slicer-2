...
namespace ORNL {
RomiFanucWriter::RomiFanucWriter(GcodeMeta meta, const QSharedPointer<SettingsBase>& sb) : WriterBase(meta, sb) {}
...
QString RomiFanucWriter::writeInitialSetup(Distance minimum_x, Distance minimum_y, Distance maximum_x,
    ...
    if (m_sb->setting<int>(PRS::GCode::kEnableStartupCode)) {
        rv += "T2" % m_newline % "M06" % m_newline % "G90G00G54X0.Y0." % m_newline % "G43H2Z100." % m_newline;
    }
...
QString RomiFanucWriter::writeBeforePart(QVector3D normal) {
    QString rv;
    return rv;
}

QString RomiFanucWriter::writeBeforeIsland() {
    QString rv;
    return rv;
}

QString RomiFanucWriter::writeBeforeRegion(RegionType type, int pathSize) {
    QString rv;
    return rv;
}

QString RomiFanucWriter::writeBeforePath(RegionType type) {
    QString rv;
...
QString RomiFanucWriter::writeTravel(Point start_location, Point target_location, TravelLiftType lType,
...
QString RomiFanucWriter::writeArc(const Point& start_point, const Point& end_point, const Point& center_point,
...
QString RomiFanucWriter::writeAfterPath(RegionType type) {
    QString rv;
    if (!m_spiral_layer) {
        rv += writeExtruderOff();
        if (type == RegionType::kPerimeter) {
...
QString RomiFanucWriter::writeAfterRegion(RegionType type) {
    QString rv;
    return rv;
}

QString RomiFanucWriter::writeAfterIsland() {
    QString rv;
    return rv;
}

QString RomiFanucWriter::writeAfterPart() {
    QString rv;
    return rv;
}

QString RomiFanucWriter::writeAfterLayer() {
    QString rv;
...
QString RomiFanucWriter::writePurge(int RPM, int duration, int delay) {
    return "M69 F" % QString::number(RPM) % m_p % QString::number(duration) % m_s % QString::number(delay) %
           commentSpaceLine("PURGE");
}

QString RomiFanucWriter::writeDwell(Time time) {
    if (time > 0)
...
QString RomiFanucWriter::writeCoordinates(Point destination) {
    QString rv;
