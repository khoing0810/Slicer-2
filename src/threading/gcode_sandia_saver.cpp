...
void GCodeSandiaSaver::run() {
    ...
    for (int i = 0; i < lines.size(); i++) //(QString line : lines)
    {
        ...
        if (line.startsWith(G0)) {
            ...
            QString aval = QString::number(GSM->getGlobal()->setting<Angle>(PRS::MachineSetup::kAxisA)());
            QString bval = QString::number(GSM->getGlobal()->setting<Angle>(PRS::MachineSetup::kAxisB)());
            QString cval = QString::number(GSM->getGlobal()->setting<Angle>(PRS::MachineSetup::kAxisC)());
            if (GSM->getGlobal()->setting<int>(ES::FileOutput::kSandiaCVEL)) {
                out << "LIN {X " % xval % ", Y " % yval % ", Z " % zval % ", A " % aval % ", B " % bval % ", C " %
                           cval % ", E1 0.000, E2 0.000, E3 0.000, E4 0.000, E5 0.000, E6 0.000 } C_VEL" % newline;
            }
            else {
                out << "LIN {X " % xval % ", Y " % yval % ", Z " % zval % ", A " % aval % ", B " % bval % ", C " %
                           cval % ", E1 0.000, E2 0.000, E3 0.000, E4 0.000, E5 0.000, E6 0.000 } C_DIS" % newline;
            }
        }
